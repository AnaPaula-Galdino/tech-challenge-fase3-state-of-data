...
if __name__ == "__main__":
    ...
    for e in E.ORDEM_EDICOES:
        ...
        # sem round intermediario: o rotulo ja arredonda uma vez, e arredondar
        # duas vezes deslocaria o valor
        prio.append(float(sub["participacao_pct"].sum()))
    nao = [float(ia[(ia.edicao == e) & (ia.categoria == "Não é prioridade")]["participacao_pct"].iloc[0]) for e in E.ORDEM_EDICOES]
    gerados.append(linha_por_edicao(
        ...
        "Percentual de respondentes por posição declarada da empresa sobre IA generativa: de 36,2% para 60,6%, alta de 68%, e de 28,8% para 11,3%, queda de 61%. Bases: 896, 1.045 e 652 respostas válidas",
    ...
    E.moldura(ax, "Python lidera nas três edições, e o SQL aparece ao lado dele em 2025-2026",
              "Percentual de respondentes que citam a linguagem como preferida. Até 2024-2025 a pergunta admitia uma só resposta, e o SQL aparecia em 2,0% e 2,1%. Em 2025-2026 passou a admitir várias, por isso o salto do SQL para 84,2% é mudança de formato, não de mercado",
    ...
    E.moldura(ax, "Engenharia de dados e machine learning lideram a mediana salarial",
              "Mediana do ponto médio da faixa salarial declarada, por cargo, edição 2025-2026. Omitidos os cargos com menos de 30 respostas e a categoria Outra Opção, que agrupa 206 respostas sem cargo identificável. Valores nominais",
    ...
    # a categoria "sem preferencia" e a maior em nuvem e a segunda em BI. Omiti-la
    # do grafico mantendo o denominador cheio distorceria a leitura, por isso ela
    # aparece como barra de contexto, no cinza, e nao entra no ranking
    sem_nuv = "Não sei opinar / Não tenho preferência"
    sem_bi = "Não tenho preferência / Não sei opinar"
    top_nuv = ["Amazon Web Services (AWS)", "Google Cloud (GCP)", "Azure (Microsoft)", sem_nuv]
    top_bi = ["Microsoft PowerBI", "Tableau", "Looker", sem_bi]
    ...
    rot_nuv = ["AWS", "Google\nCloud", "Azure", "Sem\npreferência"]
    rot_bi = ["Power BI", "Tableau", "Looker", "Sem\npreferência"]
    ...
    for eixo, rotulos, valores, tit in [(a1, rot_nuv, v_nuv, "Nuvem preferida"),
        ...
        cores = [E.AZUL_ESCURO, E.AZUL_CLARO, E.AZUL_CLARO, E.CINZA_CONTEXTO]
        eixo.bar(rotulos, valores, color=cores, width=0.62, zorder=3)
    ...
    E.moldura(a2, "AWS lidera na nuvem e o Power BI lidera em BI, mas um terço não tem preferência",
              "Preferência declarada de nuvem e de ferramenta de BI, edição 2025-2026, sobre 2.095 e 1.866 respostas válidas. A pergunta é de preferência, não de uso. Considerando apenas quem declara preferência, AWS chega a 47,0% e Power BI a 65,6%",
              fonte=FONTE_2025)
